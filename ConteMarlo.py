...
import pypokerengine.utils.card_utils
from pypokerengine.utils.card_utils import _pick_unused_card, _fill_community_card, _montecarlo_simulation
...
def rando_betting(equity, valid_actions):
    amount = None
    fold_prob = normpdf(equity, mean = 20, sd = 10)
    call_prob = normpdf(equity, mean = 50, sd = 80)
    raise_prob = normpdf(equity, mean = 75, sd = 15)

    #Scale
    total = fold_prob + call_prob + raise_prob
    fold_prob = fold_prob / total
    call_prob = call_prob / total
    raise_prob = raise_prob / total

    probs = [fold_prob, fold_prob + call_prob]

    #Generate random number b/w 0 and 1
    val = random.uniform(0,1)

    if val < fold_prob + call_prob:
        action = 'call'
        ...
        amount = min((min_raise / 2) * 3, max_raise)
    else:
        action = 'fold'
...
class ConteMarloBot(BasePokerPlayer):
    def __init__(self):
        self.num_players = 0
        self.wins = 0
        self.losses = 0
